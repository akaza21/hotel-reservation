...
// AddUserWithCredentials creates a user with specific email and password
func AddUserWithCredentials(store *db.Store, fn, ln, email, password string, admin bool) *types.User {
	user, err := types.NewUserFromParams(types.CreateUserParams{
		Email:     email,
		FirstName: fn,
		LastName:  ln,
